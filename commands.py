...
<b><u>Подробнее:</u></b>''']
    ...
    if 'rights' in report.keys():
        for el in report['rights']:
            msg.append(f"Номер: {el['number'] if 'number' in el.keys() else '-'}")
            msg.append(f"Дата: {el['date'] if 'date' in el.keys() else '-'}")
            msg.append(f"Тип: {el['type'] if 'type' in el.keys() else '-'}")

            if 'limits' in el.keys():
                msg.append('<b><u>Ограничения:</u></b>')
                for limit in el['limits']:
                    msg.append(f"   Номер: {limit['number'] if 'number' in el.keys() else '-'}")
                    msg.append(f"   Дата: {limit['date'] if 'date' in el.keys() else '-'}")
                    msg.append(f"   Тип: {limit['type'] if 'type' in el.keys() else '-'}")
                    msg.append("\n")
            else:
                msg.append('<b><u>Ограничения: -</u></b>')
    ...
    if len(result_msg) > 4095:
        return '<b>Внимание! Неполный отчёт - слишком большое сообщение</b>\n' + result_msg[:2000]
    return result_msg
